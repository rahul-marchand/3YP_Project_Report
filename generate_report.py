...
ue_ann_contrib  = UE.cell(35, 9).value
ue_3yr_ltv      = UE.cell(42, 9).value
ue_ss_ltv       = UE.cell(44, 9).value
ue_cac          = UE.cell(51, 9).value
ue_ltv_cac_3    = UE.cell(54, 9).value
ue_ltv_cac_ss   = UE.cell(55, 9).value
ue_payback      = UE.cell(56, 9).value
...
def run_scenario(selector):
    ...
    s = w2["Scenarios"]
    result = {
        "name":    s.cell(27, 2).value,
        "revenue": s.cell(28, 2).value,
        "gp":      s.cell(29, 2).value,
        "ebitda":  s.cell(30, 2).value,
        "ni":      s.cell(31, 2).value,
        "cash":    s.cell(32, 2).value,
        "trough":  s.cell(33, 2).value,
        "break":   s.cell(34, 2).value,
        "equity":  s.cell(35, 2).value,
    }
    # Also extract EBITDA trajectory for chart overlay
    pl = w2["P&L"]
...
print("Running scenarios (base / downside / upside / reg delay)...")
SCENARIO_RESULTS = {
    "Base":     run_scenario(1),
...
# Tier mix is keyed to phases, not years (Y1-Y3, Y4-Y5, Y6-Y7)
# Mix is Standard/Pro (institutional subscription only — individuals do not subscribe)
tier_mix_by_year = [
    "60/40", "60/40", "60/40",
    "70/30", "70/30",
    "80/20", "80/20",
...
# Operating expenses → Capex. Italic sub-headers separate the groups.
groups = [
    ("\\textit{Revenue}", [
        ("New units sold (units)",              "\\cref{chapter:ent-gtm}",
         [fmt_num(v) for v in new_units]),
        ("Hardware ASP (\\pounds)",             "\\cref{sec:ent-unit-economics}",
         [fmt_num(300) for _ in range(N)]),
        ("Subscription attach rate (\\%)",      "\\cref{sec:ent-unit-economics}",
         [fmt_pct(A.cell(33, 2).value or 0.50) for _ in range(N)]),
        ("Annual churn (\\%)",                  "\\cref{sec:ent-unit-economics}",
         [fmt_pct(A.cell(16, 2).value or 0.10) for _ in range(N)]),
        ("Tier mix (Std/Pro, \\%)",             "this section",
         tier_mix_by_year),
        ("Active subscribers (\\emph{derived})", "prose",
    ...
    ("\\textit{Operating expenses}", [
        ...
        ("Marketing (\\pounds k)",              "\\cref{chapter:ent-gtm}",
         [fmt_num(v/1000) for v in marketing]),
        ("G\\&A non-payroll (\\pounds k)",      "T\\&S, prof fees, IT, facilities, insurance",
...
for i, (group_label, rows) in enumerate(groups):
    if i > 0:
        body_lines.append("        \\addlinespace[0.3em]")
    body_lines.append(f"        \\multicolumn{{{2+N}}}{{l}}{{{group_label}}} \\\\")
    for label, source, vals in rows:
        body_lines.append(
            "        \\quad " + label + " & " + source + " & " + " & ".join(vals) + " \\\\"
        )
body = "\n".join(body_lines)
...
conventions_groups = [
    ("\\textit{Revenue and COGS}", [
        ("Returns allowance",
            f"{returns*100:.0f}\\% of gross hardware revenue",
            "\\cref{sec:ent-unit-economics}; DOA + trial-period + RMA"),
        ("Subscription COGS",
            f"{sub_cogs*100:.0f}\\% of subscription revenue",
            "cloud, support, data egress"),
        ("Accessory gross margin",
            f"{acc_gm*100:.0f}\\%",
            "\\cref{sec:ent-operations}"),
    ...
    ("\\textit{Payroll}", [
        ("Founder partial cash",
            ...
            "vs full salary; founders partial cash pre-Series A"),
    ]),
    ("\\textit{Working capital}", [
        ("DIO / DSO / DPO",
    ...
    ("\\textit{Tax}", [
        ...
        ("R\\&D qualifying split",
            ...
            "eng/reg/clin/trial/non-payroll; cash refund in year"),
    ]),
    ("\\textit{Accounting conventions}", [
        ("Depreciation",
            "Straight-line; 3y IT/bench, 5y tooling/fit-out",
...
for i, (group_label, conv_rows) in enumerate(conventions_groups):
    if i > 0:
        conv_body_lines.append("        \\addlinespace[0.3em]")
    conv_body_lines.append(f"        \\multicolumn{{3}}{{l}}{{{group_label}}} \\\\")
    for label, value, source in conv_rows:
        conv_body_lines.append(
            "        \\quad " + label + " & " + value + " & " + source + " \\\\"
        )
...
# Margin metrics for the summary panel
gm_y4            = gross_profit[3] / total_rev[3] if total_rev[3] else 0
gm_y9            = gross_profit[6] / total_rev[6] if total_rev[6] else 0
ebitda_margin_y9 = ebitda[6] / total_rev[6] if total_rev[6] else 0
recurring_y9     = (sub_rev[6] + acc_rev[6] + warranty_rev[6]) / total_rev[6] if total_rev[6] else 0
cum_losses       = sum(min(0, ni) for ni in net_income[:7])  # Y1-Y7 trading losses
op_lev_y9        = ((ebitda[6] - ebitda[5]) / (total_rev[6] - total_rev[5])) if (total_rev[6] - total_rev[5]) else 0

pnl_summary_body = "\n".join([
    f"        Gross margin (Y4 commercial launch $\\to$ Y7 steady state) & {gm_y4*100:.0f}\\% $\\to$ {gm_y9*100:.0f}\\% \\\\",
    f"        EBITDA margin (Y7) & {ebitda_margin_y9*100:.0f}\\% \\\\",
    f"        Recurring revenue share of total (Y7) & {recurring_y9*100:.0f}\\% \\\\",
    f"        Cumulative trading losses (Y1--Y7, prior to break-even) & {fmt_gbp(cum_losses)} \\\\",
    f"        Y7 incremental EBITDA margin (operating leverage) & {op_lev_y9*100:.0f}\\% \\\\",
])
...
cum_def_rev_contrib = sum(def_rev_change)
cum_wc_contrib      = sum(total_wc_change)
cash_conv_y9        = ncfo[6] / ebitda[6] if ebitda[6] else 0

summary_body = "\n".join([
    f"        Peak unfunded cash trough (binding funding requirement) & {fmt_gbp(peak_trough)} ({peak_trough_yr}) \\\\",
    f"        Cumulative deferred-revenue contribution to cash (Y1--Y7) & {fmt_gbp(cum_def_rev_contrib)} \\\\",
    f"        Cumulative net working-capital contribution (Y1--Y7) & {fmt_gbp(cum_wc_contrib)} \\\\",
    f"        Y7 cash conversion (NCFO $\\div$ EBITDA) & {cash_conv_y9*100:.0f}\\% \\\\",
    f"        Total capex through horizon & {fmt_gbp(total_capex_cf)} \\\\",
    f"        First year of self-funded positive cash & {first_selffund} \\\\",
])
...
# ---- Table 4: Unit economics ------------------------------------------------
ue_body = "\n".join([
    f"        Hardware contribution per unit          & {fmt_gbp(ue_hw_contrib)} \\\\",
    f"        Subscription contribution per customer per year & {fmt_gbp(ue_sub_contrib)} \\\\",
    f"        Annual contribution per active customer & {fmt_gbp(ue_ann_contrib)} \\\\",
    f"        3-year total LTV per customer           & {fmt_gbp(ue_3yr_ltv)} \\\\",
    f"        Steady-state LTV per customer (perpetuity) & {fmt_gbp(ue_ss_ltv)} \\\\",
    f"        CAC per new customer                    & {fmt_gbp(ue_cac)} \\\\",
    f"        LTV:CAC ratio (3-year)                  & {ue_ltv_cac_3:.1f}$\\times$ \\\\",
    f"        LTV:CAC ratio (steady state)            & {ue_ltv_cac_ss:.1f}$\\times$ \\\\",
    f"        Payback period                          & {ue_payback:.1f}\\,months \\\\",
])
