...
Three choices configure the segmentation pipeline: backbone, loss, and preprocessing. Each choice is evidenced by ablation on OpenEDS~\cite{garbin2019openeds} (12,759 images, 152 subjects). Every configuration is trained for 50 epochs on an NVIDIA RTX A4000 with Adam ($\eta = 10^{-3}$) and ReduceLROnPlateau scheduling. Compute constraints limited us to 3 seeds on two reference configurations and single seeds elsewhere; the measured $\pm$0.03\,px seed-to-seed variance sets the uncertainty floor for comparing single-seed runs. We report pupil IoU for comparability with prior work but select on \emph{axis error}, since IoU saturates on the bulk of interior pixels whereas axis error reflects the pupil--iris boundary precision that determines downstream ellipse-fit quality.
...
\paragraph{Backbone: RITnet.} U-Net matches RITnet on axis error (0.13 vs 0.09\,px, within seed variance) at 28$\times$ the model size (29.6 vs 1.07\,MB). We pick RITnet for its efficiency.

\paragraph{Loss: compound RITnet loss.} CE, Dice, CE + Dice, and the compound RITnet loss are indistinguishable within seed variance. We adopt the compound loss from the original RITnet paper~\cite{chaudhary2019ritnet} for consistency with the backbone's published configuration.
...
\paragraph{Interpreting the result.} The 7.9 and 6.5\,px baselines are dominated by a ${\sim}10\%$ subset of frames in which the model misclassifies image-corner artefacts as sclera, the ellipse fit lands on the corner, and centre error spikes. This is a discrete failure mode, not a noisy estimate. Excluding frames with error >10\,px, augmentation still improves accuracy on surviving frames (baseline 1.5\,px $\to$ standard 1.2\,px), so the headline change combines a smaller accuracy improvement with a failure-mode rescue. The failure frames are retained in the average because a 10\% complete-failure rate is real failure rather than measurement noise; the key finding is that augmentation removes every observation of this failure mode.
...
From the segmentation mask we need clinical \ab{plr} biomarkers. This requires solving three problems in sequence: extracting a per-frame measurement, cleaning the time series of frame-level artefacts, and fitting biomarkers to the cleaned trajectory. We select against the constraints set out at the start of \Cref{sec:pupil-width}.
...
\begin{figure}[htbp]
    ...
    \caption{Parametric \ab{plr} fit and noise robustness. (a)~Parametric model (red, $R^2 = 0.965$) fitted to a synthetic \ab{plr} waveform with $\sigma = 0.2$\,mm noise; biomarkers are read analytically from fitted parameters of \Cref{eq:plr-model}. (b)~Parameter recovery vs noise level over 100 synthetic trials per condition: relative errors in amplitude, latency, recovery $\tau$, and the fit residual $1 - R^2$.}
