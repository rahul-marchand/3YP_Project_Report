...
Separate machine learning models were fine-tuned for each of these biomarkers. For smooth pursuit, a convolutional neural network (CNN) was developed to predict gaze angle from an eye image, helping to assess whether a user can follow a marker on a screen properly. For the pupil light reflex biomarker, a different CNN model measures pupil width, giving insight into pupil dilation responses when exposed to light by producing a binary mask showing whether a pixel belongs to the pupil or not.
...
The module also outputs two quality-related quantities. Motion validity is derived from IMU-based gating logic and indicates whether excessive head movement compromised the trial. Signal confidence reflects the reliability of the eye-tracking signal, derived from analysis of blur, blinking, and glare. The trial is disqualified if the IMU acceleration exceeds 1.5\,g for more than 300\,ms, or if more than 10\% of frames are rejected by the quality pipeline. These provisional thresholds will be revised against real headset data.
...
\Cref{fig:gaussian-optimisation} shows that the Gaussian filter performs the best as it has the lowest error in saccade predictions. However, it still required optimisation, so a grid search was performed of the better Gaussian filters, achieving the filter in \Cref{fig:optimal-gaussian}. The original unoptimised parameters were $\sigma_g = 1.5$ and threshold~$=$~30\,deg/s, giving MAE~$=$~12.2. Raising threshold to 50\,deg/s caught the real saccades, modelled as 120--240\,deg/s velocity peaks. These test results are evidence of the validity of this methodology.
