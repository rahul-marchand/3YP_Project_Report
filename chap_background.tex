...
This chapter establishes the scientific and technical foundation for the screening system presented in this report. It begins by defining \ab{mtbi} and the pathological mechanisms that make it difficult to detect with conventional imaging (\Cref{sec:bg-mtbi}). It then reviews existing diagnostic approaches and their limitations for field deployment (\Cref{sec:bg-existing}), before presenting the oculomotor biomarkers that motivate the system design (\Cref{sec:bg-oculomotor}). Finally, it surveys prior engineering work on smartphone-based oculomotor assessment (\Cref{sec:bg-engineering}).
...
Brainstem and midbrain structures demonstrate particular susceptibility to \ab{dai} due to their anatomical positioning at the interface between the rigid skull base and mobile cerebral hemispheres. These regions house critical nuclei for autonomic and oculomotor control, including the oculomotor nucleus, Edinger-Westphal nucleus (controlling pupillary constriction), and mesencephalic reticular formation (controlling vergence eye movements). The midbrain's narrow dimensions and sensitivity to rotational loading make it particularly vulnerable, with oculomotor nerve fibres and the medial longitudinal fasciculus traversing this region. This vulnerability provides the basis for the high prevalence of oculomotor dysfunction following \ab{mtbi}~\cite{hirad2019neural}.
...
Multiple studies have demonstrated diagnostic accuracy comparable to or exceeding conventional clinical tools (see \Cref{sec:bg-oculomotor} for specific results). Modern smartphone front-facing cameras provide 8--12 megapixel resolution at 30--60 fps, sufficient for tracking pupil diameter with sub-millimetre precision and measuring constriction velocities.

Most published research has focused on pupillary light reflex alone, rather than implementing the multi-parameter oculomotor assessment (PLR, smooth pursuit, vergence) that clinical evidence suggests provides superior diagnostic performance~\cite{mcdonald2022eye}. Furthermore, translating from research validation to systems usable by untrained personnel at the point of injury requires additional work in user interface design, automated analysis, and lay-user validation.
